...
# Find UUID in 3x-ui
INBOUND_DATA=$(curl -sf -b "$COOKIE" "$XUI_HOST/xui/API/inbounds/get/${INBOUND_ID}")
UUID=$(python3 - <<EOF
import sys, json
try:
...
if [[ -n "$UUID" ]]; then
    ...
    echo "WARN: Client not found in 3x-ui (may already be removed)"
fi

# Deactivate subscription token
